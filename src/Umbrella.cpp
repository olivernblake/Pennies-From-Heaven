...
//--------------------------------------------------------
void Umbrella::setup(glm::vec2 _position, string _hand)  {
    width = ofGetWidth();
    height = ofGetHeight();
    hand = _hand;
    umbrellaSize = width/8;
    umbrellaHeight = umbrellaSize/2.5;
    poleCol = ofColor(180);
    umbrellaPos = _position;
    // Shift down slightly so appears holding Poles;
    holdPoles = umbrellaSize / 2.5;
    umbrellaPos.y = umbrellaPos.y + holdPoles;
...
//--------------------------------------------------------------
void Umbrella::update(glm::vec2 _position) {
    // Update Position;
    umbrellaPos = _position;
...
void Umbrella::draw(){
    ...
    // Sensor Across Umbrella Lip;
    ofPushStyle();
    ofSetColor(255, 0); // For DEBUG;
    ofSetRectMode(OF_RECTMODE_CENTER);
    ofDrawRectangle(0, 0, umbrellaSize, sensorHeight * 2);
    ofPopStyle();
    
    ofPopMatrix();
}
//--------------------------------------------------------------
void Umbrella::updatePos() {
...
//--------------------------------------------------------------
void Umbrella::setUmbrellaDrawInfo() {
